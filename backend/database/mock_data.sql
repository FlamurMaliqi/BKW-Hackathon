...
INSERT INTO projects (name, description, start_date, deadline, status, priority, completion_percent, budget_total, budget_spent) VALUES
    ('Solar Roof Expansion', 'PV capacity increase across municipal buildings', '2025-12-01', '2026-04-15', 'planning', 'high', 0.00, 1250000, 125000),
    ('District Heating Retrofit', 'Modernise district heating control loops for downtown grid', '2025-10-01', '2025-12-20', 'active', 'medium', 32.00, 890000, 285000),
    ('Battery Storage Pilot', 'Pilot-scale storage system for peak shaving', '2025-11-10', '2026-02-28', 'planning', 'medium', 5.00, 640000, 32000),
    ('Hydro Plant Modernization', 'Turbine control upgrade and monitoring rollout', '2025-09-15', '2025-11-30', 'active', 'high', 58.00, 1560000, 905000),
    ('HVAC Optimization Program', 'Energy efficiency improvements for office portfolio', '2025-11-20', '2026-02-15', 'planning', 'low', 3.00, 540000, 16000);
...
INSERT INTO project_assignments (engineer_id, project_id, hours_per_week, start_date, end_date) VALUES
    ((SELECT id FROM engineers WHERE email = 'anna.keller@bkw.ch'), (SELECT id FROM projects WHERE name = 'Solar Roof Expansion'), 25, '2025-12-01', '2026-04-01'),
    ((SELECT id FROM engineers WHERE email = 'anna.keller@bkw.ch'), (SELECT id FROM projects WHERE name = 'Hydro Plant Modernization'), 10, '2025-09-15', '2025-11-30'),
    ((SELECT id FROM engineers WHERE email = 'lukas.meier@bkw.ch'), (SELECT id FROM projects WHERE name = 'District Heating Retrofit'), 28, '2025-10-01', '2025-12-15'),
    ((SELECT id FROM engineers WHERE email = 'sofia.baumann@bkw.ch'), (SELECT id FROM projects WHERE name = 'District Heating Retrofit'), 18, '2025-10-01', '2025-12-20'),
    ((SELECT id FROM engineers WHERE email = 'sofia.baumann@bkw.ch'), (SELECT id FROM projects WHERE name = 'HVAC Optimization Program'), 12, '2025-11-20', '2026-02-10'),
    ((SELECT id FROM engineers WHERE email = 'marco.steiner@bkw.ch'), (SELECT id FROM projects WHERE name = 'Battery Storage Pilot'), 22, '2025-11-10', '2026-02-28'),
    ((SELECT id FROM engineers WHERE email = 'laura.brunner@bkw.ch'), (SELECT id FROM projects WHERE name = 'Solar Roof Expansion'), 18, '2025-12-01', '2026-04-10'),
    ((SELECT id FROM engineers WHERE email = 'laura.brunner@bkw.ch'), (SELECT id FROM projects WHERE name = 'HVAC Optimization Program'), 12, '2025-11-20', '2026-02-10'),
    ((SELECT id FROM engineers WHERE email = 'jonas.frei@bkw.ch'), (SELECT id FROM projects WHERE name = 'Hydro Plant Modernization'), 35, '2025-09-15', '2025-11-30'),
    ((SELECT id FROM engineers WHERE email = 'jonas.frei@bkw.ch'), (SELECT id FROM projects WHERE name = 'Battery Storage Pilot'), 10, '2025-11-10', '2026-02-20')
